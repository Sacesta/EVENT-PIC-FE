...
export default function ProducerLogin() {
  ...
  useEffect(() => {
    if (!isLoadingUser && user) {
      if (user.role === 'producer') {
        ...
        // If not a producer, redirect to appropriate dashboard
        const dashboardUrl = user.role === 'admin' ? "/admin-dashboard" : "/supplier-dashboard";
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Allow both producer and admin users to login
      if (responseData.data.user.role !== 'producer' && responseData.data.user.role !== 'admin') {
        toast({
          title: t('common.error'),
          description: "This login page is for producers only. Please use the appropriate login page for your role.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (returnUrl && savedEventData && responseData.data.user.role === 'producer') {
        try {
      ...
      const dashboardUrl = responseData.data.user.role === 'admin' ? '/admin-dashboard' : '/producer-dashboard';
          ...
          if (errorData.errorType) {
            switch (errorData.errorType) {
              case 'EMAIL_NOT_FOUND':
                ...
                errorMessage = errorData.message || 'No producer account found with this email.';
                break;
              ...
              case 'ACCOUNT_DEACTIVATED':
                ...
                errorMessage = errorData.message || 'Your producer account has been deactivated.';
                break;
              case 'SERVER_ERROR':
        ...
        <Card className="shadow-lg">
          <CardHeader className="text-center space-y-4">
            ...
            <CardTitle className="text-3xl font-semibold text-foreground">
              {t('auth.producerLogin.title')}
            </CardTitle>
            <p className="text-muted-foreground">
              {t('auth.producerLogin.subtitle')}
            </p>
          </CardHeader>
          ...
          <CardContent>
            ...
            <div className="mt-6 pt-4 border-t border-border/50">
              <p className="text-sm text-muted-foreground text-center">
                {t('auth.producerLogin.noAccount')}{' '}
                <Link 
                ...
                >
                  {t('auth.producerLogin.registerHere')}
                </Link>
              </p>
