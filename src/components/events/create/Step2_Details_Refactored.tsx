...
const Step2_Details: React.FC<Step2_DetailsProps> = React.memo(({ 
  ...
  const handlePrivateToggle = useCallback((enabled: boolean) => {
  ...
  const handlePaidToggle = useCallback((enabled: boolean) => {
  ...
  const handleUpdateTicket = useCallback((id: string, updates: Partial<Ticket>) => {
  ...
  const handleRemoveTicket = useCallback((id: string) => {
    const updatedTickets = eventData.tickets.filter(ticket => ticket.id !== id);
    handleUpdate('tickets', updatedTickets);
  }, [eventData.tickets, handleUpdate]);
