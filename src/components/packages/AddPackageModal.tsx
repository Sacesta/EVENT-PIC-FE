...
import apiService, { type PackageData } from '@/services/api';
...
export default function AddPackageModal({ isOpen, onClose, onPackageAdded }: AddPackageModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Name is required and must be 2-200 chars
    ...
    // Build the package data object, excluding image initially
    ...
    const packageDataToSend = formData.image
      ? { ...basePackageData, image: formData.image }
      : basePackageData;

    console.log('Submitting package data:', packageDataToSend);

    setIsLoading(true);
    try {
      // Create package using new /packages backend endpoint
      const response = await apiService.createPackage(packageDataToSend as PackageData & { image?: File });
      console.log('Package creation response:', response);

      toast({
        title: t('common.success', 'Success'),
        description: t('packages.packageCreated', 'Package created successfully')
      });
      onPackageAdded();
      onClose();
      // Reset form
      ...
      let errorMessage = t('packages.createError', 'Failed to create package. Please try again.');
      try {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {t('packages.addPackageDescription', 'Create a new package offering for your clients')}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                        formData.availability?.days?.includes(day)
                          ...
                          : 'bg-muted hover:bg-muted/80 text-muted-foreground dark:bg-muted/20 dark:hover:bg-muted/30'
                      }`}
                    >
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <Button type="submit" disabled={isLoading}>
              {isLoading ? t('common.creating', 'Creating...') : t('packages.createPackage', 'Create Package')}
            </Button>
          </div>
        </form>
      </DialogContent>
